...
class jd_spider(Spider):
    name = "jd"
    start_urls = []
    for i in range(1, 820):   # 这里请自己设置页数，目前只能抓取电子烟分类下前10页的商品
        # url = 'http://list.jd.com/list.html?cat=670,671,672&page=' + str(i) +'&delivery=1&sort=sort_dredisprice_desc&trans=1&JL=4_4_0#J_main'
        # url = 'http://list.jd.com/list.html?cat=670,671,672&jth=yi&page=' + str(i)
        # url = 'http://list.jd.com/list.html?cat=1672,2599,1440&ev=111217_635585&page=' + str(i)
        url = 'http://list.jd.com/list.html?cat=670,671,672&page=' + str(i)
        print "***"+url+"***"
